...
funtopol<-function(int_data){
  ...
  if ("Canopy" %in% names(int_data) == FALSE) stop("ERROR: your interactions data lacks a column named: Canopy")
  if ("Recruit" %in% names(int_data) == FALSE) stop("ERROR: your interactions data lacks a column named: Recruit")
  if ("Open" %in% int_data$Canopy == FALSE) stop("ERROR: your data does not contain a node named Open or it is spelled differently.")
  ...
  for(i in 1:coreSize) {outReachFromCore[[i]] <- igraph::subcomponent(g, MembersCore[i], mode="out")}
  ...
  classif <- list(
    ...
    MembersDdTransients)
  classif <- setNames(classif, c("Satellites",
                                 "Core",
                                 "Strict_transients",
