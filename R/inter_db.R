...
inter_db <- function(data, dbcover) {
  ...
  site_surf <-
    data.frame(unique(dbcover[, c("Study_site", "Plot", "Sampled_distance_or_area")]) %>%
                 group_by(Study_site) %>%
  ...
  rmnets_anyNA <-
    unique(data[data$mynet_sp %in% setdiff(unique(data$mynet_sp), (Canopy_all$mycover_sp)), "Study_site"])

  dbcover <-
    droplevels(dbcover[dbcover$Study_site %in% setdiff(dbcover$Study_site, rmnets_anyNA), ])
  data <-
    droplevels(data[data$Study_site %in% setdiff(data$Study_site, rmnets_anyNA), ])
  Canopy_all <-
    droplevels(Canopy_all[Canopy_all$Study_site %in% setdiff(Canopy_all$Study_site, rmnets_anyNA), ])

  inter <- data.frame(
    data %>%
      group_by(Study_site, Recruit, Canopy, inter_ID) %>%
      summarise(
        inter_ID = unique(inter_ID),
  ...
  for (z in 1:length(unique(Canopy_all$Study_site))) {
    ...
    alledge <-
      merge(Freq, unique(myadj[, c("Study_site", "inter_ID", "Canopy", "Recruit")]), by = "inter_ID")
    myadj <-
      dcast(data = alledge[, c("Recruit", "Canopy", "Freq")], Recruit ~ Canopy, value.var =
              "Freq")
    ...
    obs_inter <-
      data.frame(expand.grid(dimnames(provideDimnames(myadj)))[1:2], as.vector(as.matrix(myadj)))

    colnames(obs_inter)[which(colnames(obs_inter) == "Var1")] <-
      "Recruit"
    colnames(obs_inter)[which(colnames(obs_inter) == "Var2")] <-
      "Canopy"
    colnames(obs_inter)[which(colnames(obs_inter) == "as.vector.as.matrix.myadj..")] <-
      "Freq"
    ...
    #add Open cover
    myedge$Open_cover <-
      rep(mycovs[mycovs$Canopy == "Open", "Canopy_cover"], dim(myedge)[1])

    #add number or recruits in open per recruit sp

    myedge <- merge(myedge, openedge[, c("Recruit", "Freq")], by = "Recruit", all.x = TRUE)
    ...
    myedge$RII <-
      with(myedge, ((Canopy_Freq / Canopy_cover) - (Open_Freq / Open_cover)) /
