...
#' If available, the dimensions of the study plots can be also included as variables PlotdimX and PlotdimY.
...
#' (e.g. 0.01m2) but there is one sapling growing beneath, we would estimate a density of 100 saplings/m2
...
#'
#' @examples
#' int_data <- read.csv("RecruitNet.csv")
#' int_data <- comm_subset(RecruitNet, site = "Ventisquero")
#' cover_data <- read.csv("CanopyCover.csv")
#' cover_data <- cover_data[cover_data$Study_site == "Ventisquero",]
#' Ventisquero_int <- associndex(int_data, cover_data)
#'
#'
associndex <- function(int_data=NULL, cover_data=NULL, area_sampled=NULL, thr_dens=100){

  if(is.null(area_sampled) & "PlotdimX" %in% names(int_data) == FALSE) stop("ERROR: sampled area cannot be calculated with the data available. You must enter a value in area_sampled or add variables PlotdimX and PlotdimY in your data.")

  area_sampled <- ifelse(is.null(area_sampled), area_sampled <- comm_summary(int_data)["Area sampled (m2)"][1,1], area_sampled)
